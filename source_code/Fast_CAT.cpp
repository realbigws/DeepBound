...
void WS_Fast_Cat(string &list,string &root,string &out)
{
	//read in list
	ifstream fin;
	string buf,temp;
	...
	for(i=0;i<totnum;i++)
	{
		string file=root+"/"+ws_list_rec[i];
		WS_Read_In_Cat(file,ws_total_rec[i]);
	}
	//======= final output =====//
...
int main(int argc,char **argv)
{
	//--------- WS_FAST_Cat -----------//
	{
		if(argc<4)
		{
			printf("Fast_CAT <list> <root> <out> \n");
			exit(-1);
		}
		string list=argv[1];
		string root=argv[2];
		string out=argv[3];
		WS_Fast_Cat(list,root,out);
		exit(0);
	}
}
