...
//		int l2=0;
	...
	feat_str_tot.clear();
	//-> add half_wind = -200
	Average_Value_Vector(feat_mat, feat_str_cur, -200);
	feat_str_tot.push_back(feat_str_cur);
	//-> add half_wind = -100
	Average_Value_Vector(feat_mat, feat_str_cur, -100);
	feat_str_tot.push_back(feat_str_cur);
	//-> add half_wind = -50
...
int main(int argc,char **argv)
{
	//---- Mingfu_Feat_Merge_wind ----//
	{
		if(argc<5)
		{
			...
			fprintf(stderr,"[wind]: the output will use -200,-105,-50,0,+50,+100,+200 half_wind \n");
			exit(-1);
		}
		//argument
