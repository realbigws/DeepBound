...
int abundance_labels[12] = {15, 22, 33, 48, 68, 95, 132, 181, 255, 362, 530, 932};
	...
	vector <vector <double> > &feature_)
{
	...
	for(int k=0;k<(int)feature[0].size();k++)
	{
...
int main(int argc,char **argv)
{
	//------- Mingfu_Feature --------// 
	{
		if(argc<7)
		{
			fprintf(stderr,"Mingfu_Feature_v3 <raw_input> <root_feat> <root_cov> <root_l1> <root_l2> <root_l3> \n");
			fprintf(stderr,"[note]: root_l1 is for 'Start Mid End' (0,1,2) labels \n");
			fprintf(stderr,"        root_l2 is for abundance original value \n");
			fprintf(stderr,"        root_l3 is for 13 abundance labels \n"); 
			exit(-1);
		}
		...
		vector <vector <int> > label_abu;
		vector <vector <vector <double> > > feature_tot;
		Read_All(raw_input,label_tot,label_abu,feature_tot);
		//output
