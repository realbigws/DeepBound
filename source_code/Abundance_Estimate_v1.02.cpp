...
void Estimate_Abundance_Value(vector < vector < double > > &abun_prob,
	vector <double> &nnreg_value, vector <double> &cov_value)
{
	//-> estimate distance
	...
	{
		//check label_0
		if(abun_prob[i][0]>0.8 && cov_value[i]<2)
		{
			printf("0\n");
			continue;
		}
		...
		{
			lower_bound=estimated_abun-2.0*estimated_std1;
			upper_bound=12;
		}
		else
		{
			lower_bound=estimated_abun-2.0*estimated_std1;
			upper_bound=estimated_abun+2.0*estimated_std2;
		}
...
int main(int argc,char **argv)
{
	//---- Abundance_Estimate ----//__170101__//
	{
		if(argc<4)
		{
			fprintf(stderr,"Abundance_Estimate <abundance_prob> <nnreg_value> <cov_value> \n");
			exit(-1);
		}
		//argument
		string abundance_prob=argv[1];
		string nnreg_value=argv[2];
		string cov_value=argv[3];
		//load feature
		...
		int gg=Load_NNreg_Abudance(cov_value, cov_val);
		//check length
		if(len!=ll || len!=gg)
		{
			fprintf(stderr,"len %d not equal to ll %d or gg %d \n",len,ll,gg);
			exit(-1);
		}
		//abundance estimate
		Estimate_Abundance_Value(abun_prob,nnreg_val,cov_val);
		exit(0);
	}
}
